...
const AssignedStudents = () => {
  ...
  const getStatusText = (status) => {
    switch (status) {
      ...
      default:
        return status ? status.charAt(0).toUpperCase() + status.slice(1).replace("_", " ") : "Pending";
    }
  };
  ...
  const submitFeedback = () => {
    if (selectedProject?._id && feedbackData.title && feedbackData.message) {
      dispatch(addFeedback({ projectId: selectedProject._id, feedback: feedbackData }));
      closeModal();
    }
  };
  ...
  const handleAddMember = async () => {
    ...
    try {
      await axiosInstance.post(`/teacher/projects/${selectedProject._id}/add-member`, {
  ...
  const handleRemoveMember = async () => {
    ...
    setActionLoading(true);
    try {
      await axiosInstance.put(`/teacher/projects/${selectedProject._id}/remove-member`, {
  ...
  const handleChangeLeader = async () => {
    ...
    setActionLoading(true);
    try {
      await axiosInstance.put(`/teacher/projects/${selectedProject._id}/change-leader`, {
  ...
  const handleSplitProject = async () => {
    ...
    setActionLoading(true);
    try {
      await axiosInstance.post(`/teacher/projects/${selectedProject._id}/split`, {
  ...
  const filteredProjects = useMemo(() => {
    ...
    // Sort
    if (sortBy === "size") {
      result.sort((a, b) => (a.groupMembers?.length || 0) - (b.groupMembers?.length || 0));
    } else {
      result.sort((a, b) => {
  ...
  const stats = [
    { label: "Total Projects", value: projects.length, bg: "bg-blue-50", text: "text-blue-700", sub: "text-blue-600" },
    { label: "Completed", value: projects.filter((p) => p.status === "completed").length, bg: "bg-green-50", text: "text-green-700", sub: "text-green-600" },
    { label: "In Progress", value: projects.filter((p) => p.status === "approved" || p.status === "in_progress").length, bg: "bg-yellow-50", text: "text-yellow-700", sub: "text-yellow-600" },
    { label: "Total Students", value: assignedStudents?.length || 0, bg: "bg-purple-50", text: "text-purple-700", sub: "text-purple-600" },
  ];

  if (loading) return <Loader className="animate-spin w-16 h-16 mx-auto mt-12" />;
  if (error) return <div className="text-center py-10 text-red-600 font-medium">Error loading students</div>;
    ...
    <>
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            {stats.map((item) => (
              <div className={`${item.bg} rounded-lg p-4`} key={item.label}>
                ...
                <p className={`${item.text} text-2xl font-bold`}>{item.value}</p>
              </div>
            ))}
          </div>
        ...
        {activeTab === "projects" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {filteredProjects.map((project) => {
              ...
              return (
                <div className="card hover:shadow-lg transition-all duration-300 flex flex-col" key={project._id}>
                    ...
                    <div className="flex-1 mr-4">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`shrink-0 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                          isGroup ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500"
                        }`}>
                          {isGroup ? "Group" : "Individual"}
                        </span>
                      </div>
                      <p className="text-sm text-slate-500">
                        {project.groupName || (isGroup ? "Group Project" : "Individual Project")}
                      </p>
                    </div>

                    <div className="flex items-center gap-2">
                      <span className={`shrink-0 px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadge(project.status)}`}>
                      ...
                      <div className="relative">
                        <button
                          ...
                          onClick={() => setShowCustomizeMenu(showCustomizeMenu === project._id ? null : project._id)}
                        >
                        ...
                        {showCustomizeMenu === project._id && (
                          <div className="absolute right-0 mt-1 w-52 bg-white rounded-lg shadow-xl border border-slate-200 z-20 py-1">
                            <button
                              ...
                              onClick={() => openCustomizeAction(project, "add")}
                            >
                            ...
                            {isGroup && (
                              <>
                                <button
                                  className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2.5"
                                  onClick={() => openCustomizeAction(project, "remove")}
                                >
                                ...
                                <button
                                  className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2.5"
                                  onClick={() => openCustomizeAction(project, "leader")}
                                >
                            ...
                            {isGroup && project.groupMembers.length >= 3 && (
                              <>
                                ...
                                <button
                                  className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2.5"
                                  onClick={() => openCustomizeAction(project, "split")}
                                >
        ...
        {activeTab === "students" && (
          <div className="card">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
              <h2 className="text-lg font-semibold text-slate-800">All Supervised Students</h2>
              <div className="relative">
                      ...
                      <tr key={student._id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-4">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            isLeader ? "bg-indigo-100 text-indigo-800" : "bg-slate-100 text-slate-800"
                          }`}>
                            {isLeader ? "Leader" : "Member"}
                          </span>
                        </td>
                        <td className="p-4 text-sm text-slate-600 max-w-xs truncate" title={student.project?.title}>
                          {student.project?.title || "-"}
                        </td>
                        <td className="p-4">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                            mode === "group" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500"
                          }`}>
        ...
        {showFeedbackModal && selectedProject && (
          <div className="fixed inset-0 bg-black backdrop-blur-sm bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={closeModal}>
            <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-slate-800">Provide Feedback</h2>
                <button className="text-slate-400 hover:text-slate-600 bg-slate-100 rounded-full p-1" onClick={closeModal}>
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="bg-slate-50 rounded-lg p-4 mb-6 border border-slate-100">
                <div className="space-y-3 text-sm">
                  <div className="flex flex-col">
                    <span className="text-slate-500 text-xs uppercase tracking-wider font-semibold mb-1">Project</span>
                    <span className="text-slate-800 font-medium">{selectedProject.title || "No title"}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-slate-500 text-xs uppercase tracking-wider font-semibold mb-1">Team</span>
                    <span className="text-slate-800">{selectedProject.groupMembers.map((m) => m.name).join(", ")}</span>
                  </div>
                </div>
              </div>
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">Feedback Message</label>
                  <textarea
              ...
              <div className="flex gap-3 mt-8">
                <button className="flex-1 px-4 py-2.5 text-slate-700 bg-slate-100 hover:bg-slate-200 font-medium rounded-lg" onClick={closeModal}>Cancel</button>
                <button
        ...
        {showCompleteModal && selectedProject && (
          <div className="fixed inset-0 bg-black backdrop-blur-sm bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={closeModal}>
            <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-slate-800">Confirm Completion</h2>
                <button className="text-slate-400 hover:text-slate-600 bg-slate-100 rounded-full p-1" onClick={closeModal}>
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                <div className="space-y-2 text-sm">
                  <div className="flex flex-col">
                    <span className="font-semibold text-yellow-800 mb-1">Project</span>
                    <span className="text-yellow-900">{selectedProject.title || "No title"}</span>
                  </div>
                  <div className="flex flex-col mt-3">
                    <span className="font-semibold text-yellow-800 mb-1">Team</span>
                    <span className="text-yellow-900">{selectedProject.groupMembers.map((m) => m.name).join(", ")}</span>
                  </div>
                </div>
              </div>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={closeModal}>
            <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-6">
                ...
                <button className="text-slate-400 hover:text-slate-600" onClick={closeModal}><X className="w-5 h-5" /></button>
              </div>
              <p className="text-sm text-slate-600 mb-1">
                Project: <strong>{selectedProject.title}</strong>
              </p>
              <p className="text-sm text-slate-500 mb-4">Select an unassigned student to add:</p>
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {orphanStudents.length === 0 && (
                  <p className="text-slate-400 text-sm text-center py-4">No unassigned students available.</p>
                )}
                {orphanStudents.map((s) => (
                  <label
                    key={s._id}
                    className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                      selectedStudentId === s._id ? "border-green-500 bg-green-50" : "border-slate-200 hover:bg-slate-50"
                    }`}
                  >
                    <input type="radio" name="addStudent" checked={selectedStudentId === s._id} onChange={() => setSelectedStudentId(s._id)} />
                    <div>
              ...
              <div className="flex gap-3 mt-6">
                <button className="flex-1 px-4 py-2.5 bg-slate-100 text-slate-700 font-medium rounded-lg hover:bg-slate-200" onClick={closeModal}>Cancel</button>
                <button
            ...
            <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {selectedProject.groupMembers
                  .filter((m) => {
                    const leaderId = selectedProject.student?._id || selectedProject.student;
                  ...
                  .map((m) => (
                    <label
                      key={m._id}
                      className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                        selectedStudentId === m._id ? "border-orange-500 bg-orange-50" : "border-slate-200 hover:bg-slate-50"
                      }`}
                    >
                      <input type="radio" name="removeMember" checked={selectedStudentId === m._id} onChange={() => setSelectedStudentId(m._id)} />
                      <div>
                ...
                {selectedProject.groupMembers
                  ...
                  .map((m) => (
                    <label
                      key={m._id}
                      className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                        selectedStudentId === m._id ? "border-blue-500 bg-blue-50" : "border-slate-200 hover:bg-slate-50"
                      }`}
                    >
                      <input type="radio" name="newLeader" checked={selectedStudentId === m._id} onChange={() => setSelectedStudentId(m._id)} />
                      <div>
                        <p className="font-medium text-slate-800">{m.name}</p>
                        <p className="text-sm text-slate-500">{m.email}</p>
                      </div>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={closeModal}>
            <div className="bg-white rounded-xl shadow-2xl p-6 max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="text-sm text-slate-500 mb-4">
                Select members to move to the new project. The remaining members stay in the original.
              </p>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">New Project Title</label>
                <input
                  type="text"
              ...
              <p className="text-sm font-medium text-slate-700 mb-2">Move these members to the new project:</p>
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {selectedProject.groupMembers.map((m) => (
                  <label
                    key={m._id}
                    className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                      splitMembers.includes(m._id) ? "border-purple-500 bg-purple-50" : "border-slate-200 hover:bg-slate-50"
                    }`}
                  >
                    <input
                      type="checkbox"
              ...
              {splitMembers.length > 0 && splitMembers.length < selectedProject.groupMembers.length && (
                <div className="mt-3 p-3 bg-purple-50 rounded-lg text-sm text-purple-800 border border-purple-200">
                  <strong>Original:</strong> {selectedProject.groupMembers.filter((m) => !splitMembers.includes(m._id)).map((m) => m.name).join(", ")} →{" "}
                  <strong>New:</strong> {selectedProject.groupMembers.filter((m) => splitMembers.includes(m._id)).map((m) => m.name).join(", ")}
                </div>
              )}
              <div className="flex gap-3 mt-6">
                <button className="flex-1 px-4 py-2.5 bg-slate-100 text-slate-700 font-medium rounded-lg hover:bg-slate-200" onClick={closeModal}>Cancel</button>
                <button
                  ...
                  disabled={splitMembers.length === 0 || splitMembers.length >= selectedProject.groupMembers.length || actionLoading}
