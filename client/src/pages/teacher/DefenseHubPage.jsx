...
const getProjectDisplayName = (project) =>
  project?.groupName || project?.title || "N/A";
  ...
  switch (filterKey) {
    ...
    case "other":
      return records.filter((record) =>
        ["excused", "absent"].includes(record.status),
      );
    case "all":
    default:
      return records;
...
const DefenseHubPage = () => {
  ...
  const loadData = async () => {
    ...
    try {
      const [
        ...
        axiosInstance
          .get("/teacher/ngrok-url")
          .catch(() => ({ data: { data: { publicUrl: null } } })),
      ]);
      ...
      setTeachers(usersRes.data.data?.teachers || []);
      setNgrokBaseUrl(ngrokRes.data.data?.publicUrl || "");

      const projectIds = Array.from(
        new Set(
          nextStudents.map((student) => student.project?._id).filter(Boolean),
        ),
      );
        ...
        projectIds.map(async (projectId) => {
          try {
            const res = await axiosInstance.get(
              `/teacher/projects/${projectId}/assessment-summary`,
            );
      ...
      toast.error(
        error.response?.data?.message || "Failed to load defense hub",
      );
    } finally {
      setLoading(false);
    }
  ...
  const selectedAssessmentProject = useMemo(
    () =>
      projectOptions.find(
        (project) => project._id === selectedAssessmentProjectId,
      ) ||
      projectOptions[0] ||
      null,
  ...
  const createSchedule = async () => {
    ...
    if (!scheduleForm.slots.every((slot) => slot.startAt && slot.endAt)) {
      toast.error(
        "Every slot must include both defense start time and end time",
      );
      return;
    }
  ...
  const createAttendance = async () => {
    ...
    if (!attendanceForm.startsAt || !attendanceForm.endsAt) {
      toast.error(
        "Please fill in both meeting start time and meeting end time",
      );
      return;
    }
  ...
  const submitScore = async (councilId, projectId) => {
    ...
    try {
      await axiosInstance.post(
        `/teacher/councils/${councilId}/projects/${projectId}/score`,
        {
  ...
  const openEditSession = (session) => {
    setEditSessionForms((prev) => ({
      ...
      [session._id]: {
        ...
        windowMinutes:
          session.checkInClosesAt && session.startsAt
            ? Math.round(
                (new Date(session.checkInClosesAt) -
  ...
  const refreshAssessmentSummary = async (projectId) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to refresh assessment summary",
      );
    }
  };

  const updateAssessmentFormEntry = (
    projectId,
    milestoneCode,
    cloCode,
    field,
  ...
  const seedAssessmentFormFromSummary = (
    projectId,
    milestoneCode,
    summary,
    assessorId = null,
  ) => {
    const milestone = summary?.milestones?.find(
      (item) => item.code === milestoneCode,
    );
    const existingSubmission = milestone
      ? assessorId
  ...
  const submitTeacherAssessment = async (projectId, milestoneCode) => {
    const formKey = `${projectId}:${milestoneCode}`;
    const form = assessmentForms[formKey] || {
    ...
    payload.append(
      "cloEntries",
      JSON.stringify(buildRubricPayload(form.entries)),
    );
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message ||
          `Failed to save ${milestoneCode} rubric`,
      );
    }
  };
  ...
  const submitTeacherM5Assessment = async (councilId, projectId) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to submit M5 rubric",
      );
    }
  };
  ...
  const reviewM6Submission = async (
    projectId,
    submissionId,
    approvalStatus,
  ) => {
    ...
    payload.append(
      "overallComment",
      m6ReviewForms[submissionId]?.overallComment || "",
    );
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to review M6 submission",
      );
    }
  };
  ...
  const finalizeCloAssessmentAction = async (councilId, projectId) => {
    ...
    const currentM5Submission = selectedAssessmentSummary?.milestones
      ...
      ?.assessorSubmissions?.find(
        (submission) =>
          toEntityId(submission.assessor) === toEntityId(authUser?._id),
      );
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to finalize CLO assessment",
      );
    }
  };
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-lg p-6 text-white">
        ...
        <p className="text-emerald-100">
          Manage defense schedule slots, attendance sessions, reviewer
          assignment, scoring, and final lock.
        </p>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <p className="card-subtitle">
            Follow the steps in order so the flow is clear for both teacher and
            student.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="card space-y-4">
          <div className="card-header">
            <h2 className="card-title">Section A. Create Defense Schedule</h2>
            <p className="card-subtitle">
              Create one slot-picking batch for supervised teams. The times
              below are defense presentation times that students will choose
              from.
            </p>
          </div>

          <FieldBlock
            label="A1. Defense Schedule Title"
          ...
          >
            <input
              ...
              onChange={(event) =>
                updateScheduleField("title", event.target.value)
              }
            />
          </FieldBlock>
          ...
          >
            <textarea
              ...
              onChange={(event) =>
                updateScheduleField("description", event.target.value)
              }
            />
          </FieldBlock>
          ...
          >
            <input
              ...
              onChange={(event) =>
                updateScheduleField("pickDeadline", event.target.value)
              }
            />
          </FieldBlock>
            ...
            >
              <div className="border-b border-slate-200 pb-2">
                ...
                <p className="text-xs text-slate-500">
                  One slot equals one concrete defense appointment that one team
                  can book.
                </p>
              </div>
              ...
              >
                <input
                  ...
                  value={slot.startAt}
                  onChange={(event) =>
                    updateSlotField(index, "startAt", event.target.value)
                  }
                />
              </FieldBlock>
              ...
              >
                <input
                  ...
                  value={slot.endAt}
                  onChange={(event) =>
                    updateSlotField(index, "endAt", event.target.value)
                  }
                />
              </FieldBlock>
              ...
              >
                <input
                  ...
                  value={slot.location}
                  onChange={(event) =>
                    updateSlotField(index, "location", event.target.value)
                  }
                />
              </FieldBlock>
              ...
              >
                <select
                  ...
                  value={slot.mode}
                  onChange={(event) =>
                    updateSlotField(index, "mode", event.target.value)
                  }
                >
        ...
        <div className="card space-y-4">
          <div className="card-header">
            ...
            <p className="card-subtitle">
              Create one attendance event for one supervised project. These time
              fields are meeting/check-in times, not defense slot-picking times.
            </p>
          </div>
          ...
          <div className="rounded-lg bg-slate-50 p-4 text-sm text-slate-700">
            <p className="font-medium text-slate-800 mb-1">
              Auto-generated Attendance Timing Preview
            </p>
            <p>
              Check-in opens:{" "}
              {attendanceCheckInOpen
                ? formatDateTime(attendanceCheckInOpen)
                : "Select meeting start time first"}
            </p>
            <p>
              Check-in closes:{" "}
              {attendanceCheckInClose
                ? formatDateTime(attendanceCheckInClose)
                : "Select meeting start time first"}
            </p>
          </div>
      ...
      <div className="card">
        <div className="card-header">
          <h2 className="card-title">
            Section C. Existing Defense Schedule Windows
          </h2>
        </div>
        <div className="space-y-4">
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Slot picking deadline:{" "}
                    {formatDateTime(schedule.pickDeadline)}
                  </p>
                  <p className="text-sm text-slate-500">
                    Reschedule lock: {schedule.rescheduleWindowHours} hours
                    before booked defense time
                  </p>
                ...
                <button
                  className="btn-outline"
                  onClick={() => runAutoAssign(schedule._id)}
                >
                ...
                {(schedule.slots || []).map((slot) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        Location: {slot.location || "No location"} | Mode:{" "}
                        {formatModeLabel(slot.mode)}
                      </p>
                    </div>
          ...
          {schedules.length === 0 && (
            <p className="text-slate-500">No defense schedules created yet.</p>
          )}
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="card-title">
                Section D. Attendance Sessions and Leave Review
              </h2>
              <p className="card-subtitle">
                Review live student check-in status, pending confirmations, and
                leave requests.
              </p>
            </div>
        ...
        <div className="space-y-4">
              ...
              >
                  ...
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2 mb-0.5">
                        ...
                        <button
                          onClick={() =>
                            deleteSession(session._id, session.title)
                          }
                      ...
                      <p className="text-sm text-slate-500">
                        Meeting time: {formatDateTime(session.startsAt)} -{" "}
                        {formatDateTime(session.endsAt)}
                      </p>
                      <p className="text-sm text-slate-500">
                        Check-in window:{" "}
                        {formatDateTime(session.checkInOpensAt)} -{" "}
                        {formatDateTime(session.checkInClosesAt)}
                      </p>
                      <p className="text-sm text-slate-500">
                ...
                {editSessionForms[session._id]?.open && (
                  <div className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-slate-600">
                          Check-in Close Window (min after start)
                        </label>
                        <input
                          className="input"
                          type="number"
                          min="5"
                          value={
                            editSessionForms[session._id]?.windowMinutes || 15
                          }
                          onChange={(e) =>
                            updateEditSessionField(
                              session._id,
                    ...
                    <p className="text-xs text-blue-700">
                      Check-in opens 15 min before start. Saving recomputes the
                      session status automatically.
                    </p>
                    <div className="flex gap-2 pt-1">
                      <button
                        className="btn-primary"
                        onClick={() => saveEditSession(session._id)}
                      >
                        Save Changes
                      </button>
                      <button
                        className="btn-outline"
                        onClick={() => closeEditSession(session._id)}
                      >
                        Cancel
                      </button>
                    </div>
                ...
                <div className="mb-4 rounded-lg bg-slate-50 border border-slate-200 p-4">
                  ...
                  <p className="text-sm text-slate-500 mb-3">
                    Students should scan while signed in to their student
                    account. The QR opens the attendance page and the app
                    confirms attendance with the temporary token automatically.
                  </p>
                  {sessionQrCodes[session._id] ? (
                    ...
                    <p className="text-sm text-slate-500">
                      QR is being generated or could not be generated for this
                      session.
                    </p>
                  )}
                </div>
                    ...
                    {attendanceFilterOptions.map((option) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            updateAttendanceFilter(session._id, option.key)
                          }
                        >
                          {option.label}
                        </button>
                  ...
                  {visibleRecords.map((record) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-slate-500">
                          Manual override allowed if student has technical
                          problems or late confirmation from teacher.
                        </p>
          ...
          {sessions.length === 0 && (
            <p className="text-slate-500">No attendance session created yet.</p>
          )}
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          <h2 className="card-title">
            Section E. Council Reviewer Assignment and Scoring
          </h2>
          <p className="card-subtitle">
            Chairman assigns reviewer. Then council members and reviewer submit
            scores and comments.
          </p>
        </div>
        <div className="space-y-4">
              ...
              {(council.projects || []).map((projectItem) => {
                ...
                return (
                  <div
                    ...
                    className="rounded-lg bg-slate-50 p-4 mb-3"
                  >
                    <p className="font-medium text-slate-800">
                    ...
                    <p className="text-sm text-slate-500 mb-3">
                      Reviewer:{" "}
                      {projectItem.reviewer?.name || "Not assigned yet"} |
                      Weighted score: {projectItem.weightedAverage ?? "N/A"}
                    </p>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-slate-500">
                          Only the chairman should assign the reviewer for each
                          project in this council.
                        </p>
                        <select
                          className="input"
                        ...
                        <input
                          ...
                          step="0.1"
                          value={
                            reviewerAssignments[key]?.reviewerWeight || 1.5
                          }
                          onChange={(event) =>
                            updateReviewerAssignment(
                              key,
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-slate-500">
                          Each council member enters their own score and
                          comment. Chairman locks the final result after review.
                        </p>
                        <input
                          className="input"
                          type="number"
                          ...
                          onChange={(event) =>
                            updateScoreForm(key, "score", event.target.value)
                          }
                        />
                        <textarea
                          ...
                          onChange={(event) =>
                            updateScoreForm(key, "comment", event.target.value)
                          }
                        />
                        <textarea
                          className="input min-h-20"
                        ...
                        <div className="flex gap-2">
                          <button
                            className="btn-primary"
                            onClick={() =>
                              submitScore(council._id, projectItem.project?._id)
                            }
                          >
                            Save Score
                          </button>
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-slate-500">
                          The assigned reviewer writes the report here. The
                          system exports it as a PDF file for project records.
                        </p>
                        <textarea
                          className="input min-h-20"
      ...
      <div className="card space-y-4">
        <div className="card-header">
          ...
          <p className="card-subtitle">
            Score milestones M1-M6 by CLO rubric, review peer/ICS, and watch QA
            readiness before finalizing.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(event) =>
                setSelectedAssessmentProjectId(event.target.value)
              }
            >
              <option value="">Select supervised project</option>
              {projectOptions.map((project) => (
          ...
          <div className="rounded-lg bg-slate-50 p-4">
            ...
            <p className="font-semibold text-slate-800">
              {selectedAssessmentSummary?.projectTrack ||
                selectedAssessmentProject?.projectTrack ||
                "capstone"}{" "}
              |{" "}
              {formatAssessmentScore(
                selectedAssessmentSummary?.teamFinalScore,
                "/10",
              )}{" "}
            ...
            <p className="text-sm text-slate-500">
              QA completeness:{" "}
              {selectedAssessmentSummary?.qaEvidenceSummary
        ...
        {!selectedAssessmentProject && (
          <p className="text-slate-500">
            Choose a supervised project to work with milestone CLO scoring.
          </p>
        )}
          ...
          <div className="space-y-4">
            {["M1", "M2", "M3"].map((milestoneCode) => {
              ...
              const milestoneSummary =
                selectedAssessmentSummary?.milestones?.find(
                  (item) => item.code === milestoneCode,
                );
              const form = assessmentForms[formKey] || {
                entries: createRubricEntries(),
                  ...
                  <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        Score:{" "}
                        {formatAssessmentScore(
                          milestoneSummary?.componentScore5,
                          "/5",
                        )}{" "}
                        | Status: {milestoneSummary?.status || "pending"}
                      </p>
                    </div>
                    <button
                      className="btn-outline"
        ...
        {selectedAssessmentProject && assessmentTab === "M4" && (
          <div className="rounded-lg border border-slate-200 p-4 space-y-4">
            <div>
              <p className="font-semibold text-slate-800">
                M4. {MILESTONE_LABELS.M4}
              </p>
              <p className="text-sm text-slate-500">
                Use this section for report / thesis rubric before the defense
                milestone opens.
              </p>
            </div>
            ...
            <RubricTable
              entries={
                (
                  assessmentForms[`${selectedAssessmentProject._id}:M4`] || {
                    entries: createRubricEntries(),
                  }
                ).entries
              }
            ...
            <button
              className="btn-primary"
              onClick={() =>
                submitTeacherAssessment(selectedAssessmentProject._id, "M4")
              }
            >
              Save M4 Rubric
            </button>
        ...
        {assessmentTab === "M5" && (
          <div className="space-y-4">
                  ...
                  >
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      <div>
                        ...
                        <p className="text-sm text-slate-500">
                          {council.name} | Team result:{" "}
                          {formatAssessmentScore(
                            assessmentSummary?.teamFinalScore,
                            "/10",
                          )}
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                      <div className="rounded-lg bg-slate-50 p-4">
                        ...
                        <div className="space-y-2">
                          {(
                            assessmentSummary?.milestones?.find(
                              (item) => item.code === "M5",
                            )?.requiredAssessors || []
                          ).map((assessor, index) => (
                            <div
                              key={`${assessor.teacher?._id || assessor.teacher}-${index}`}
                      ...
                      <div className="xl:col-span-2 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 rounded-lg bg-slate-50 p-4">
                          ...
                          <div>
                            ...
                            <p className="font-medium text-slate-700">
                              {assessmentSummary?.qaEvidenceSummary
                                ?.completenessPercent || 0}
                              %
                            </p>
                          </div>
                          <div>
                            ...
                            <p className="font-medium text-slate-700">
                              {assessmentSummary?.qaEvidenceSummary?.missingItems?.join(
                                ", ",
                              ) || "None"}
                            </p>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          <button
                            className="btn-primary"
                            onClick={() =>
                              submitTeacherM5Assessment(council._id, projectId)
                            }
                          >
            ...
            {councils.length === 0 && (
              <p className="text-slate-500">
                No councils available for M5 scoring.
              </p>
            )}
          </div>
        )}
        ...
        {selectedAssessmentProject && assessmentTab === "M6" && (
          <div className="space-y-4">
            <div className="rounded-lg bg-slate-50 p-4">
              <p className="font-semibold text-slate-800">
                M6. {MILESTONE_LABELS.M6}
              </p>
              <p className="text-sm text-slate-500">
                Students submit peer / ICS evidence first. Supervisor approves
                or rejects each submission here.
              </p>
            </div>

            {(selectedAssessmentSummary?.studentAssessments || []).map(
              (studentAssessment) => {
                const reviewSubmissionId =
                  studentAssessment.reviewSubmissionId ||
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      <div>
                        ...
                        <p className="text-sm text-slate-500">
                          M6 score:{" "}
                          {formatAssessmentScore(
                            studentAssessment.individualM6Score5,
                            "/5",
                          )}{" "}
                          | Final:{" "}
                          {formatAssessmentScore(
                            studentAssessment.officialFinalScore,
                            "/10",
                          )}{" "}
                      ...
                      <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
                        {studentAssessment.peerSubmission?.approvalStatus ||
                          "No submission"}
                      </span>
                    </div>

                    {studentAssessment.peerSubmission?.cloEntries?.length >
                      0 && (
                      <div className="overflow-x-auto rounded-lg border border-slate-200">
                        <table className="min-w-full text-sm">
                          ...
                          <tbody className="divide-y divide-slate-200 bg-white">
                            {studentAssessment.peerSubmission.cloEntries.map(
                              (entry) => (
                                <tr
                                  key={`${studentAssessment.student?._id}-${entry.cloCode}`}
                                >
                    ...
                    <textarea
                      ...
                      value={
                        m6ReviewForms[reviewSubmissionId]?.overallComment || ""
                      }
                      onChange={(event) =>
                        updateM6ReviewForm(
                          reviewSubmissionId,
            ...
            {(selectedAssessmentSummary?.studentAssessments || []).length ===
              0 && (
              <p className="text-slate-500">
                No student peer / ICS submissions yet.
              </p>
            )}
          </div>
        )}
        ...
        {selectedAssessmentProject && assessmentTab === "SUMMARY" && (
          <div className="space-y-4">
            ...
            <div className="rounded-lg border border-slate-200 p-4">
              ...
              <div className="space-y-2">
                {(selectedAssessmentSummary?.milestones || []).map(
                  (milestone) => (
                    <div
                      key={milestone.code}
            ...
            <div className="rounded-lg border border-slate-200 p-4">
              ...
              <p className="text-sm text-slate-500">
                {selectedAssessmentSummary?.qaEvidenceSummary?.missingItems?.join(
                  ", ",
                ) || "No missing items"}
              </p>
            </div>
