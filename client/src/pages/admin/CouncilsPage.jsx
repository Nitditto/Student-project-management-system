...
const CouncilsPage = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [usersRes, projectsRes, councilsRes, templatesRes, qaRes] =
        await Promise.all([
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-blue-700 rounded-lg p-6 text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-indigo-100">
            Admin creates the council, assigns chairman and secretary, then
            attaches approved-completed projects.
          </p>
        </div>
        <button
        ...
        <div className="card space-y-4">
          <div className="card-header">
            ...
            <p className="card-subtitle">
              Follow CLO achievement, evidence completeness, and projects that
              still need QA attention.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            ...
            <div className="rounded-lg border border-slate-200 p-4">
              <p className="mb-3 font-medium text-slate-700">
                Projects Requiring QA Follow-up
              </p>
              <div className="space-y-2">
                {displayedProjectWarnings.map((item) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-600">
                      CLO red:{" "}
                      {item.redClos.length ? item.redClos.join(", ") : "None"} |
                      QA completeness: {item.qaCompleteness}%
                    </p>
                ...
                {(qaDashboard.projectWarnings || []).length > 5 && (
                  ...
                  >
                    {isQaExpanded
                      ? "Show Less"
                      : `Show All (${qaDashboard.projectWarnings.length})`}
                  </button>
                )}
              </div>
            </div>
              ...
              <div className="card-header">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <p className="card-subtitle">
                      {formatDateTime(council.defenseDate)} | Room:{" "}
                      {council.room || "N/A"}
                    </p>
                    {council.description && (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    >
                      <option value="">
                        Default template for selected track
                      </option>
                      {templates
                        .filter(
                          (template) =>
                ...
                <div className="space-y-2">
                  {(council.projects || []).map((projectItem) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-slate-500">
                        Reviewer:{" "}
                        {projectItem.reviewer?.name ||
                          "Waiting for chairman assignment"}
                      </p>
                      <p className="text-sm text-slate-500">
                        Track:{" "}
                        {projectItem.projectTrack ||
                          projectItem.project?.projectTrack ||
                          "capstone"}{" "}
                        | Template:{" "}
                        {projectItem.templateVersion ||
                          projectItem.assessmentSummary?.templateVersion ||
                          "default"}
                      </p>
                      <p className="text-sm text-slate-500">
                        Final weighted score:{" "}
                        {projectItem.weightedAverage ?? "N/A"} | Status:{" "}
                        {projectItem.status}
                      </p>
                      {projectItem.assessmentSummary && (
                        <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3 rounded-lg bg-slate-50 p-3">
                          <div>
                            ...
                            <p className="font-semibold text-slate-800">
                              {formatAssessmentScore(
                                projectItem.assessmentSummary.teamFinalScore,
                                "/10",
                              )}{" "}
                          ...
                          <div>
                            ...
                            <p className="font-semibold text-slate-800">
                              {projectItem.assessmentSummary.qaEvidenceSummary
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100">
            <span className="text-sm text-slate-500">
              ...
              to{" "}
              <span className="font-semibold text-slate-700">
                {Math.min(currentPage * itemsPerPage, filteredCouncils.length)}
              </span>{" "}
              of{" "}
              <span className="font-semibold text-slate-700">
                {filteredCouncils.length}
              ...
              <div className="space-y-4">
                ...
                {form.members.map((member, index) => (
                  ...
                  >
                    ...
                    <FieldBlock
                      label="Teacher"
                      hint="Select the teacher for this council position."
                    >
                      <select
                        className="input"
                    ...
                    <FieldBlock
                      label="Role"
                      hint="Set the role this teacher will hold inside the council."
                    >
                      <select
                        className="input"
                        value={member.role}
                    ...
                    <FieldBlock
                      label="Score Weight"
                      hint="This weight contributes to the final weighted council score."
                    >
                      <input
                        className="input"
                        type="number"
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-slate-500 mb-5">
                If this council has assigned projects that are not finalized
                yet, they will be detached from the council automatically.
              </p>
