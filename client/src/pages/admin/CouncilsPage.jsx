...
const CouncilsPage = () => {
  ...
  const [newSessionTemplate, setNewSessionTemplate] = useState({ startTime: "", endTime: "" });
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [usersRes, projectsRes, councilsRes, templatesRes, qaRes, jobsRes] = await Promise.all([
        ...
        axiosInstance.get("/scheduler/jobs").catch(() => ({ data: { data: [] } })),
      ]);
  ...
  useEffect(() => {
    if (projects.length > 0 && selectedProjectIds.length === 0) {
      const activeIds = projects.filter(
        p => ["approved", "in_progress", "created", "pending", "completed"].includes(p.status) && !p.councilId
      ).map(p => p._id);
  ...
  const conflictsMap = useMemo(() => {
    ...
    councils.forEach(c => {
      const memberIds = c.members?.map(m => (m.teacher?._id || m.teacher)?.toString()).filter(Boolean) || [];
      c.projects?.forEach(pItem => {
        ...
        if (supervisorId && memberIds.includes(supervisorId.toString())) {
          if (!map[c._id]) map[c._id] = { room: false, teachers: new Set(), projects: new Set(), supervisors: new Set() };
      ...
      for (let j = i + 1; j < councils.length; j++) {
        ...
        if (checkCouncilsOverlap(c1, c2)) {
          ...
          if (c1.room.trim().toLowerCase() === c2.room.trim().toLowerCase()) {
            ...
            if (!map[c2._id]) map[c2._id] = { room: false, teachers: new Set(), projects: new Set(), supervisors: new Set() };
          ...
          const getTeachers = (c) => {
            ...
            c.members?.forEach(m => {
              ...
              if (id) list.push({ id: id.toString(), name: m.teacher?.name || "Giáo viên" });
            });
            c.projects?.forEach(p => {
              const id = p.reviewer?._id || p.reviewer;
              if (id) list.push({ id: id.toString(), name: p.reviewer?.name || "Giáo viên nhận xét" });
            });
            return list;
          };
          ...
          // Project conflict
          const getProjects = (c) => (c.projects || []).map(p => {
            const id = p.project?._id || p.project;
            return { id: id?.toString(), title: p.project?.title || p.project?.groupName || "Đề tài" };
          }).filter(item => item.id);
  ...
  const saveCouncil = async () => {
    ...
    if (form.room) {
      ...
      const formTeacherIds = form.members.map(m => m.teacher?.toString() || m.teacher).filter(Boolean);
      ...
      if (editingCouncilId) {
        ...
        if (currentCouncil && currentCouncil.projects) {
          ...
          currentCouncil.projects.forEach(pItem => {
            const proj = pItem.project;
            const supervisorId = proj?.supervisor?._id || proj?.supervisor;
            if (supervisorId && formTeacherIds.includes(supervisorId.toString())) {
              const supervisorName = proj.supervisor?.name || "Giảng viên hướng dẫn";
              supervisorViolations.push(`- Giảng viên hướng dẫn "${supervisorName}" của đề tài "${proj.title || proj.groupName}" đang nằm trong danh sách thành viên hội đồng.`);
            }
          });

          if (supervisorViolations.length > 0) {
            toast.error("Không thể lưu: Giảng viên hướng dẫn không được tham gia hội đồng bảo vệ đề tài mình hướng dẫn.\n" + supervisorViolations.join("\n"));
            return;
          }
        }
      }
      ...
      if (form.defenseDate) {
        ...
        councils.forEach(c => {
          ...
          if (isTimeOverlapping) {
            ...
            if (c.room.trim().toLowerCase() === formRoom) {
              conflictMsgs.push(`- Trùng phòng: Phòng "${c.room}" đã được sử dụng cho hội đồng "${c.name}" (${new Date(c.defenseDate).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})})`);
            }
            ...
            c.members?.forEach(m => {
              const id = m.teacher?._id || m.teacher;
              if (id) cTeacherIds.push({ id: id.toString(), name: m.teacher?.name || "Giáo viên" });
            });
            c.projects?.forEach(p => {
              const id = p.reviewer?._id || p.reviewer;
              if (id) cTeacherIds.push({ id: id.toString(), name: p.reviewer?.name || "Giáo viên nhận xét" });
            });

            formTeacherIds.forEach(tId => {
              const matched = cTeacherIds.find(ct => ct.id === tId);
              if (matched) {
                conflictMsgs.push(`- Trùng giáo viên: "${matched.name}" đã tham gia hội đồng "${c.name}" (${new Date(c.defenseDate).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})})`);
              }
            });
          }
        });
        ...
        if (conflictMsgs.length > 0) {
          if (!window.confirm("Cảnh báo trùng lịch phát hiện:\n\n" + conflictMsgs.join("\n") + "\n\nBạn có chắc chắn muốn lưu không?")) {
            return;
          }
        }
      }
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message ||
          (editingCouncilId ? "Không thể cập nhật hội đồng" : "Không thể tạo hội đồng"),
      );
    }
  };
  ...
  const handleGenerateSlots = () => {
    ...
    for (let d = new Date(from); d <= to; d.setDate(d.getDate() + 1)) {
      ...
      sessionTemplates.forEach(tpl => {
        ...
        // Avoid duplicates
        const isDuplicate = timeSlots.some(s => s.startAt === startAt && s.endAt === endAt);
        if (!isDuplicate) {
  ...
  const handleAddSessionTemplate = () => {
    ...
    const isDuplicate = sessionTemplates.some(
      t => t.startTime === newSessionTemplate.startTime && t.endTime === newSessionTemplate.endTime
    );
    if (isDuplicate) {
  ...
  const handleRunScheduler = async () => {
    ...
    const runProjectIds = selectedProjectIds.filter(id => activeProjects.some(p => p._id === id));
    const runTeacherIds = selectedTeacherIds.filter(id => activeTeachers.some(t => t._id === id));
    ...
    try {
      ...
      setSolving(false);
      toast.error(error.response?.data?.message || "Không thể bắt đầu lập lịch tự động.");
    }
  };
  ...
  const handleDiscardSchedule = async () => {
    if (!jobData || !jobData._id) return;
    if (!window.confirm("Bạn có chắc chắn muốn hủy bản nháp này? Tất cả trạng thái đề tài sẽ được hoàn tác.")) return;
  ...
  const assignProject = async (councilId) => {
    ...
    if (project && council) {
      ...
      const councilTeacherIds = council.members?.map(m => (m.teacher?._id || m.teacher)?.toString()).filter(Boolean);
      if (supervisorId && councilTeacherIds.includes(supervisorId.toString())) {
        toast.error(`Không thể gán: Giảng viên hướng dẫn "${project.supervisor?.name || "GVHD"}" của đề tài này đang là thành viên của hội đồng.`);
        return;
      }
    }
        ...
        <div className="space-y-6">
          {qaDashboard && (
            <div className="card space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <div className="rounded-lg bg-slate-50 p-4">
                  <p className="text-sm text-slate-500">Hoàn thiện minh chứng QA</p>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                <div className="rounded-lg border border-slate-200 p-4">
                  <p className="mb-3 font-medium text-slate-700">Tỷ lệ đạt chuẩn đầu ra (CLO)</p>
                  <div className="space-y-2">
                ...
                <div className="rounded-lg border border-slate-200 p-4">
                  <p className="mb-3 font-medium text-slate-700">Đề tài cần cập nhật minh chứng QA</p>
                  <div className="space-y-2">
                    {displayedProjectWarnings.map((item) => (
                      <div key={item.projectId} className="rounded-lg bg-amber-50 p-3">
                        ...
                        <p className="text-sm text-slate-600">
                          CLO chưa đạt: {item.redClos.length ? item.redClos.join(", ") : "None"} | Độ hoàn thiện QA: {item.qaCompleteness}%
                        </p>
                    ...
                    {(qaDashboard.projectWarnings || []).length > 5 && (
                      ...
                      >
                        {isQaExpanded ? "Thu gọn" : `Xem tất cả (${qaDashboard.projectWarnings.length})`}
                      </button>
                    )}
                  </div>
                </div>
          ...
          <div className="space-y-4">
            {paginatedCouncils.map((council) => {
              ...
              const hasConflict = conflict && (conflict.room || conflict.teachers.size > 0 || conflict.projects.size > 0 || (conflict.supervisors && conflict.supervisors.size > 0));
                ...
                >
                  ...
                  {hasConflict && (
                    <div className="bg-red-50 border-b border-red-200 text-red-800 rounded-t-xl p-3.5 text-xs flex items-start gap-2">
                      ...
                      <div>
                        <p className="font-semibold text-sm">Phát hiện trùng lịch / vi phạm vai trò bảo vệ:</p>
                        <ul className="list-disc pl-5 mt-1 space-y-0.5">
                          {conflict.room && (
                            <li><strong>Trùng phòng:</strong> Phòng này đang được xếp lịch cho một hội đồng khác ở cùng thời gian.</li>
                          )}
                          {conflict.teachers.size > 0 && (
                            <li><strong>Trùng giáo viên:</strong> {Array.from(conflict.teachers).join(", ")} đang bận ở hội đồng khác cùng thời gian.</li>
                          )}
                          {conflict.projects.size > 0 && (
                            <li><strong>Trùng đề tài:</strong> Đề tài/nhóm {Array.from(conflict.projects).join(", ")} có lịch bảo vệ trùng thời gian ở hội đồng khác.</li>
                          )}
                          {conflict.supervisors && conflict.supervisors.size > 0 && (
                            <li><strong>Trùng vai trò:</strong> Giảng viên hướng dẫn {Array.from(conflict.supervisors).join(", ")} không được làm thành viên hội đồng cho đề tài mình hướng dẫn.</li>
                          )}
                        </ul>
                      </div>
                    </div>
                  ...
                  <div className="p-6">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                      <div>
                        <p className="font-medium text-slate-700 mb-2">Thành viên hội đồng</p>
                        <div className="space-y-2">
                          {(council.members || []).map((member) => (
                            <div
                            ...
                            >
                              <div>
                                ...
                                <p className="text-sm text-slate-500 capitalize">{member.role === "chairman" ? "Chủ tịch" : member.role === "secretary" ? "Thư ký" : "Ủy viên"}</p>
                              </div>
                              <span className="text-sm text-slate-500">Trọng số {member.weight}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        <p className="font-medium text-slate-700">Gán đề tài cho Hội đồng</p>
                        <select
                          className="input"
                    ...
                    <div className="mt-4">
                      <p className="font-medium text-slate-700 mb-2">Đề tài đã gán ({council.projects?.length || 0})</p>
                      <div className="space-y-2">
                        {(council.projects || []).map((projectItem) => (
                          <div key={projectItem.project?._id} className="rounded-lg border border-slate-200 p-3">
                            ...
                            <p className="text-sm text-slate-500">
                              GV Hướng dẫn: {projectItem.project?.supervisor?.name || "N/A"}
                            </p>
                            <p className="text-sm text-slate-500">
                              GV Phản biện: {projectItem.reviewer?.name || "Chưa phân công"}
                            </p>
                            <p className="text-sm text-slate-500">
                              Định hướng: {projectItem.projectTrack === "capstone" ? "Capstone" : "Nghiên cứu"} | Phiếu đánh giá:{" "}
                              {projectItem.templateVersion || projectItem.assessmentSummary?.templateVersion || "default"}
                            </p>
                            <p className="text-sm text-slate-500">
                              Điểm tổng hợp: {projectItem.weightedAverage ?? "Chưa có"} | Trạng thái: {projectItem.status}
                            </p>
                            {projectItem.assessmentSummary && (
                              <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3 rounded-lg bg-slate-50 p-3">
                                <div>
                                  <p className="text-xs uppercase text-slate-500">Điểm nhóm</p>
                                  <p className="font-semibold text-slate-800">
                                    {formatAssessmentScore(projectItem.assessmentSummary.teamFinalScore, "/10")} |{" "}
                                    {projectItem.assessmentSummary.teamPassStatus === "passed" ? "ĐẠT" : "CHƯA ĐẠT"}
                                  </p>
                                </div>
                                <div>
                                  <p className="text-xs uppercase text-slate-500">Độ hoàn thiện QA</p>
                                  <p className="font-semibold text-slate-800">
                                    {projectItem.assessmentSummary.qaEvidenceSummary?.completenessPercent || 0}%
                                  </p>
                                </div>
                                <div>
                                  <p className="text-xs uppercase text-slate-500">CLO rủi ro</p>
                        ...
                        {(!council.projects || council.projects.length === 0) && (
                          <p className="text-slate-500 text-xs">Chưa có đề tài nào được gán.</p>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100">
              <span className="text-sm text-slate-500">
                Hiển thị <span className="font-semibold text-slate-700">{Math.min((currentPage - 1) * itemsPerPage + 1, filteredCouncils.length)}</span> đến{" "}
                <span className="font-semibold text-slate-700">{Math.min(currentPage * itemsPerPage, filteredCouncils.length)}</span> trong số{" "}
                <span className="font-semibold text-slate-700">{filteredCouncils.length}</span> hội đồng
              </span>
        ...
        <div className="space-y-6">
          ...
          {solving && (
            <div className="card bg-blue-50 border border-blue-200 p-6 flex items-center justify-center space-y-4 flex-col text-center">
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-800">Đang chạy thuật toán tối ưu hóa</h3>
                <p className="text-slate-600 text-sm max-w-md mt-1">
                  Chạy tiến trình tính toán trong nền để tìm lịch tối ưu thỏa mãn các ràng buộc cứng.
                </p>
              </div>
              {jobData && (
            ...
            <div className="card border-2 border-emerald-500/30 space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-4 gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-slate-800">Lịch nháp tự động đã được tạo</h2>
                    <p className="text-xs text-slate-500">
                      Thời gian giải: {jobData.result?.executionTimeMs}ms | fitnessScore: {jobData.result?.fitnessScore}
                    </p>
                  </div>
                </div>
              ...
              <div>
                <h3 className="font-semibold text-slate-800 mb-3">Xem trước Hội đồng dự thảo</h3>
                    ...
                    <div key={idx} className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-3">
                      ...
                      <div className="flex flex-col gap-1 text-xs text-slate-500">
                        <span className="flex items-center gap-1.5">
                          ...
                          {getVietnameseDayOfWeek(council.defenseDate)}, ngày {new Date(council.defenseDate).toLocaleDateString("vi-VN")} vào lúc {new Date(council.defenseDate).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })}
                        </span>
                        <span className="flex items-center gap-1.5">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Thành viên</p>
                        <div className="flex flex-wrap gap-1.5">
                          {council.members?.map((m, midx) => (
                            <span key={midx} className="text-[10px] bg-white border border-slate-200 text-slate-700 px-2 py-1 rounded">
                              {m.role === "chairman" ? "Chủ tịch: " : m.role === "secretary" ? "Thư ký: " : "Ủy viên: "}
                      ...
                      <div>
                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Đề tài được phân bổ ({council.projects?.length})</p>
            ...
            <div className="space-y-6 lg:col-span-1">
              ...
              <div className="card space-y-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {rooms.length === 0 && <p className="text-xs text-slate-400">Chưa có phòng nào.</p>}
                </div>
              </div>
              ...
              <div className="card space-y-4">
                ...
                <div className="divide-y divide-slate-100 max-h-48 overflow-y-auto">
                  ...
                  {timeSlots.length === 0 && <p className="text-xs text-slate-400 py-2">Chưa có khung giờ nào.</p>}
                </div>
              </div>
            </div>
              ...
              <div className="card space-y-4">
                {(() => {
                  ...
                  const filteredProjects = activeProjects.filter(p => {
                    ...
                    const supervisorName = (p.supervisor?.name || "").toLowerCase();
                    return title.includes(search) || groupName.includes(search) || supervisorName.includes(search);
                  });
                  const allFilteredSelected = filteredProjects.length > 0 && filteredProjects.every(p => selectedProjectIds.includes(p._id));
                    ...
                    <>
                      <div className="card-header border-b border-slate-100 pb-3 flex justify-between items-center">
                        <h2 className="card-title text-sm flex items-center gap-2">
                          ...
                          Đề tài chưa xếp lịch ({selectedProjectIds.filter(id => activeProjects.some(ap => ap._id === id)).length}/{activeProjects.length})
                        </h2>
                        <button 
                          onClick={() => {
                          ...
                          <tbody className="divide-y divide-slate-100 bg-white">
                            {filteredProjects.map((proj) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-3 font-medium text-slate-800">{proj.groupName || proj.title}</td>
                            ...
                            {filteredProjects.length === 0 && activeProjects.length > 0 && (
                              <tr>
                                <td colSpan="3" className="px-4 py-8 text-center text-slate-400">
                                  Không tìm thấy đề tài nào phù hợp với từ khóa tìm kiếm.
                                </td>
                              </tr>
                            )}
                            {activeProjects.length === 0 && (
                              <tr>
                                <td colSpan="3" className="px-4 py-8 text-center text-slate-400">
                                  Không có đề tài chưa xếp lịch. Tất cả đề tài đều đã được xếp lịch hoặc chưa đủ điều kiện.
                                </td>
                              </tr>
                            )}
                          </tbody>
              ...
              <div className="card space-y-4">
                {(() => {
                  ...
                  const filteredTeachers = activeTeachers.filter(t => {
                    ...
                    const experties = (t.experties || []).map(exp => exp.toLowerCase());
                    return name.includes(search) || department.includes(search) || experties.some(exp => exp.includes(search));
                  });
                  const allFilteredSelected = filteredTeachers.length > 0 && filteredTeachers.every(t => selectedTeacherIds.includes(t._id));
                    ...
                    <>
                      <div className="card-header border-b border-slate-100 pb-3 flex justify-between items-center">
                        <h2 className="card-title text-sm flex items-center gap-2">
                          ...
                          Giảng viên khả dụng ({selectedTeacherIds.filter(id => activeTeachers.some(at => at._id === id)).length}/{activeTeachers.length})
                        </h2>
                        <button 
                          onClick={() => {
                            const filteredIds = filteredTeachers.map(t => t._id);
                            if (allFilteredSelected) {
                          ...
                          <tbody className="divide-y divide-slate-100 bg-white">
                            {filteredTeachers.map((teacher) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-3 text-slate-500">{teacher.department || "Chung"}</td>
                            ...
                            {activeTeachers.length === 0 && (
                              <tr>
                                <td colSpan="4" className="px-4 py-8 text-center text-slate-400">
                                  Không tìm thấy giảng viên hoạt động trong hệ thống.
                                </td>
                              </tr>
                            )}
                          </tbody>
          ...
          <div className="card space-y-4">
            <div className="card-header border-b border-slate-100 pb-3 flex justify-between items-center bg-slate-50/50">
              ...
              <span className="text-xs text-slate-500 font-medium">Tổng số: {schedulerJobs.length} lượt chạy</span>
            </div>
              ...
              <table className="min-w-full divide-y divide-slate-200 text-left text-xs">
                <thead className="bg-slate-50 text-slate-700 font-semibold">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right animate-none">Thao tác</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 bg-white">
                  {schedulerJobs.map((job) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3 font-medium text-slate-800">{dateStr}</td>
                        <td className="px-4 py-3 text-slate-500 capitalize">{job.config?.solverType?.replace("_", " ")}</td>
                        <td className="px-4 py-3 text-slate-500">
                          {job.config?.projectIds?.length || 0} đề tài, {job.config?.teacherIds?.length || 0} GV, {job.config?.rooms?.length || 0} phòng
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 text-slate-500">
                          {job.status === "completed" ? (
                            <span>
                              {job.result?.generatedCouncils?.length || 0} hội đồng | Fitness: {job.result?.fitnessScore || 0}
                            </span>
                          ) : job.status === "failed" ? (
                            <span className="text-red-500 truncate block max-w-xs" title={job.error}>
                              Lỗi: {job.error}
                            </span>
                          ) : (
                  ...
                  <thead className="bg-slate-50 text-slate-700 text-xs font-semibold uppercase tracking-wider">
                    <tr>
                      ...
                      {weekDates.map((day, idx) => {
                        ...
                        return (
                          <th key={idx} className={`px-4 py-3 border-r border-b border-slate-200 text-center w-52 min-w-[13rem] ${isToday ? "bg-blue-50 text-blue-700 font-bold" : ""}`}>
                            <div>{DAY_LABELS[idx]}</div>
                            <div className="text-[10px] text-slate-500 font-normal mt-0.5">({dateStr})</div>
                          </th>
                        );
                      })}
                      ...
                      <tr key={period.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        {weekDates.map((day, dayIdx) => {
                          const dayPeriodCouncils = councils.filter(c => {
                            ...
                            const matchesDay = cDate.toDateString() === day.toDateString();
                            const matchesPeriod = getPeriodFromDate(c.defenseDate) === period.id;
                            return matchesDay && matchesPeriod;
                          });

                          return (
                            <td key={dayIdx} className="px-3 py-3 border-r border-slate-200 align-top">
                              {dayPeriodCouncils.length === 0 ? (
                                ...
                                >
                                  <span className="text-slate-400 font-medium group-hover:text-slate-600 transition-colors">Trống</span>
                                ...
                                <div className="space-y-2">
                                  {dayPeriodCouncils.map((council) => {
                                    const conflict = conflictsMap[council._id];
                                    const hasConflict = conflict && (conflict.room || conflict.teachers.size > 0 || conflict.projects.size > 0 || (conflict.supervisors && conflict.supervisors.size > 0));
                                    const formattedTime = new Date(council.defenseDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                                    ...
                                    return (
                                      ...
                                      >
                                        <div className="flex items-start justify-between gap-1.5 mb-1.5">
                                          ...
                                          {hasConflict && (
                                            <AlertCircle className="w-4 h-4 text-red-600 shrink-0" title={conflict.supervisors && conflict.supervisors.size > 0 ? "Vi phạm vai trò: Giảng viên hướng dẫn trong hội đồng!" : "Phát hiện trùng lịch!"} />
                                          )}
                                        </div>
                                        ...
                                        {council.status === "draft" && !hasConflict && (
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-100 flex flex-col">
            ...
            <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50 rounded-t-xl shrink-0">
              <div>
                <h3 className="text-xl font-bold text-slate-900">
                  {editingCouncilId ? "Chỉnh sửa Hội đồng bảo vệ" : "Tạo Hội đồng bảo vệ mới"}
                </h3>
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              ...
              <div className="space-y-4">
                <p className="font-semibold text-slate-700">Thành viên hội đồng</p>
                {form.members.map((member, index) => (
                  ...
                  >
                    ...
                    <FieldBlock label="Giảng viên" hint="Chọn giảng viên đảm nhiệm vị trí này.">
                      <select
                        className="input"
                    ...
                    <FieldBlock label="Vai trò" hint="Vai trò của giảng viên trong hội đồng.">
                      <select
                        className="input"
                        value={member.role}
                      ...
                      >
                        ...
                        <option value="member">Thành viên khác (Ủy viên)</option>
                      </select>
                    </FieldBlock>
                    <FieldBlock label="Trọng số điểm" hint="Hệ số điểm đóng góp vào điểm trung bình hội đồng.">
                      <input
                        className="input"
                        type="number"
                        min="0.1"
            ...
            <div className="p-6 border-t border-slate-200 bg-slate-50 flex justify-end gap-3 rounded-b-xl shrink-0">
              {editingCouncilId && (
                <button
                  ...
                  onClick={() => {
                    const currentCouncil = councils.find(c => c._id === editingCouncilId) || { _id: editingCouncilId, name: form.name };
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-slate-500 mb-2">
                Bạn có chắc chắn muốn xóa hội đồng <strong>{councilToDelete.name}</strong>?
              </p>
              <p className="text-sm text-slate-500 mb-5">
                Các đề tài đã được gán vào hội đồng này sẽ tự động gỡ liên kết và quay về trạng thái chưa gán hội đồng.
              </p>
